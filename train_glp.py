...
from models.glp import GLP
import logging
import time


def save_checkpoint(name):
    torch.save(glp.state_dict(), osp.join(save_path, name + '.pth'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-path', default='glp')
    ...
    glp = GLP(n, edges, word_vectors.shape[1], fc_vectors.shape[1], hidden_layers, k=args.k).cuda()
    conv_time = time.time()
    word_vectors = glp.conv(word_vectors)
    ...
    optimizer = torch.optim.Adam(glp.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        if (epoch % args.save_epoch == 0):
            ...
            if args.no_pred:
                ...
                output_vectors = glp(word_vectors)
                pred_obj = {
                    'wnids': wnids,
                    'pred': output_vectors
                }
